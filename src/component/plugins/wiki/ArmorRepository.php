...
class ArmorRepository
{
    ...
    public function parseForField(?string $forJson): array
    {
        ...
        $formatStat = function ($code, $type, $val) {
            ...
            // Multiplicative types -> show as percent change (+4% for 1.04)
            if (in_array($t, ['mul', 'multiply', 'mult'], true)) {
                if (!is_numeric($num)) return (string)$val;
                $pct = ($num - 1.0) * 100.0;
                $sign = $pct > 0 ? '+' : '';
                // Format without trailing zeros when integer
                $formatted = (floor($pct) == $pct) ? (int)$pct : rtrim(rtrim(number_format($pct, 2, '.', ''), '0'), '.');
        ...
        foreach ($data as $item) {
            ...
            $extra = '';
            if (strtolower($type) === 'enchant' && is_numeric($val) && (int)$val > 0) {
                // Localized suffix like "+6 per enchant"
                $extra = '+' . (int)$val . ' ' . lang::get_phrase('per enchant');
            }
    ...
    public function getArmorBodyparts(): array
    {
        ...
        $sql = "SELECT DISTINCT bodypart FROM armors WHERE bodypart IS NOT NULL AND TRIM(bodypart) != '' AND UPPER(bodypart) NOT IN 
        ('NECK','RBRACELET','REAR;LEAR') ORDER BY bodypart";
        $res = $this->db->query($sql);
        if ($res === false) {
            return [];
        }
        return DataCache::remember('armors', 'distinct_bodyparts', function () use ($res) {
            $list = [];
            while ($row = $res->fetchArray(SQLITE3_ASSOC)) {
                $bp = strtoupper($row['bodypart']);
                if ($bp !== '') {
                    $list[] = $bp;
                }
            }
    ...
    /**
     * Return distinct jewelry body parts present in armors table.
     * @return array<string>
     */
    public function getJewelryBodyparts(): array
    {
        $jewelryParts = ['NECK','RBRACELET','REAR;LEAR','RFINGER;LFINGER','RIGHT_EAR;LEFT_EAR','RIGHT_FINGER;LEFT_FINGER','NECKLACE'];
        $in = "'" . implode("','", $jewelryParts) . "'";
        $sql = "SELECT DISTINCT bodypart FROM armors WHERE UPPER(bodypart) IN (" . $in . ") ORDER BY bodypart";
        $res = $this->db->query($sql);
        if ($res === false) {
    ...
    public function getArmorsGroupedByCrystal(string $bodypart): array
    {
        ...
        if ($bpUpper === 'JEWELRY') {
            ...
            $res = $this->db->query($sql);
        } elseif (in_array($bpUpper, $jewelryParts, true)) {
    ...
    public function getSkillById(int $skillId): ?array
    {
        ...
        $stmt = $this->db->prepare('SELECT id, name, icon, levels, overhit, isdebuff, "for" FROM skills WHERE id = :id LIMIT 1');
        ...
        foreach ($statObjects as $obj) {
            ...
            if (is_numeric($valRaw)) {
                ...
                if (in_array($tag, ['mul', 'multiply', 'mult'], true)) {
                    // multiplicative tag stores a factor like 1.04 meaning +4%
                    $pct = ($num - 1.0) * 100.0;
                    $sign = $pct >= 0 ? '+' : '';
                    $formatted = $sign . $this->trimNumber($pct) . '%';
