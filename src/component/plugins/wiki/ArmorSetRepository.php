...
class ArmorSetRepository
{
    ...
    public function getArmorById(int $id): ?array
    {
        ...
        $stmt = $this->db->prepare('SELECT id, name, icon, is_drop, is_sweep, is_craft, armor_type, bodypart, crystal_type, item_skill FROM armors WHERE id = :id LIMIT 1');
        if (!$stmt) {
    ...
    /**
     * Fetch a generic item (armor, weapon, or etcitem) by id.
     * Returns minimal fields used by templates: id, name, icon (+ item_skills for armors).
     */
    private function getItemAnyById(int $id): ?array
    {
        if ($id <= 0 || !$this->db) return null;
        // Prefer armors (to preserve item_skills parsing), then weapons, then etcitems
        // 1) Armors
        $armor = $this->getArmorById($id);
        if ($armor) return $armor;
        // 2) Weapons
        $stmt = $this->db->prepare('SELECT id, name, icon FROM weapons WHERE id = :id LIMIT 1');
        if ($stmt) {
        ...
        // 3) Etcitems
        $stmt = $this->db->prepare('SELECT id, name, icon FROM etcitems WHERE id = :id LIMIT 1');
        if ($stmt) {
            $stmt->bindValue(':id', $id, SQLITE3_INTEGER);
            $res = $stmt->execute();
            if ($res) {
    ...
    public function getSkillById(int $skillId): ?array
    {
        ...
        if (!$this->db) return null;
        // Select only columns that exist in skills table schema
        $stmt = $this->db->prepare('SELECT id, name, icon, levels, isdebuff, "for" FROM skills WHERE id = :id LIMIT 1');
        if (!$stmt) {
        ...
        if (isset($row['for']) && $row['for'] !== '' && strtolower(trim($row['for'])) !== 'null') {
            ...
            $row['formatted'] = array_map(fn($e) => $e['formatted'] ?? ($e['text'] ?? ''), $effects);
        } else {
            $row['effects'] = [];
    ...
    private function formatEffectValue(string $tag, string $stat, $val): string
    {
        ...
        if (in_array($tag, ['mul', 'multiply', 'mult'], true)) {
            // multiplicative factor (e.g. 1.04 means +4%)
            $pct = ($num - 1.0) * 100.0;
            $sign = $pct > 0 ? '+' : '';
            return $sign . $this->trimNumber($pct) . '%';
        }
    ...
    private function parseSkillList($raw): array
    {
        ...
        foreach ($skills as $s) {
            ...
            if ($meta) {
                ...
                $entry['is_debuff'] = !empty($meta['isdebuff']);
                $entry['for'] = $meta['for'] ?? null;
            ...
            // Always set icon_path (fallback to placeholder) to keep UI consistent
            $entry['icon_path'] = client_icon::getIcon($meta['icon'] ?? '', 'skills');
            $result[] = $entry;
        }
        return $result;
    }
    ...
    private function parseSingleSkill($raw): ?array
    {
        // If already an array
        if (is_array($raw)) {
            // Accept either an associative object or a list with single object
            $obj = $raw;
            if (isset($obj[0]) && is_array($obj[0]) && !isset($obj['id']) && !isset($obj['skill_id'])) {
                $obj = $obj[0];
            }
        } else {
            ...
            if (is_array($obj) && isset($obj[0]) && is_array($obj[0]) && !isset($obj['id']) && !isset($obj['skill_id'])) {
                $obj = $obj[0];
            }
        }
        ...
        // Always set icon_path (fallback to placeholder)
        $entry['icon_path'] = client_icon::getIcon($meta['icon'] ?? '', 'skills');
        return $entry;
    }
    ...
    public function getAllSetsGrouped(): array
    {
        ...
        while ($res && ($row = $res->fetchArray(SQLITE3_ASSOC))) {
            ...
            foreach ($slots as $slot) {
                ...
                // Save raw numeric IDs for template fallback rendering
                if ($ids) {
                    $tmpIds = [];
                    foreach ($ids as $idVal) if (is_numeric($idVal)) $tmpIds[] = (int)$idVal;
                    if ($tmpIds) $partsIds[$slot] = $tmpIds;
                }
                $items = [];
                foreach ($ids as $id) {
                    if (!is_numeric($id)) continue;
                    // Try to resolve from any items table (armor preferred)
                    $item = $this->getItemAnyById((int)$id);
                    if ($item) $items[] = $item;
                }
