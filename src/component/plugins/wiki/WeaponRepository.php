...
class WeaponRepository
{
    ...
    {
        // Default path inside plugin db directory (can be overridden by plugin setting)
    ...
    {
        // Use data-level cache (per DB file) to avoid repetitive SELECT DISTINCT hits
        return DataCache::remember('weapons', 'distinct_types', function () {
    ...
    $desired = ['id', 'name', 'for', 'item_skill', 'weapon_type', 'crystal_type', 'icon', 'price', 'is_magic_weapon', 'pAtk', 'mAtk', 'critRate', 'pAtkSpd', 'is_drop', 'is_sweep', 'is_craft'];
    ...
    public function countWeaponsByType(string $weaponType): int
    {
        ...
        $key = 'count_'.$weaponType;
        return DataCache::remember('weapons_counts', $key, function () use ($weaponType) {
            $stmt = $this->db->prepare('SELECT COUNT(1) AS cnt FROM weapons WHERE UPPER(weapon_type)=:t');
            if (!$stmt) return 0;
            $stmt->bindValue(':t', strtoupper($weaponType), SQLITE3_TEXT);
            $res = $stmt->execute();
    ...
    public function getWeaponsPage(string $weaponType, int $offset, int $limit): array
    {
        $offset = max(0, $offset);
        $limit  = max(1, $limit);
        $cacheKey = $weaponType.'_o'.$offset.'_l'.$limit;
        return DataCache::remember('weapons_pages', $cacheKey, function () use ($weaponType, $offset, $limit) {
            $desired = ['id','name','for','item_skill','weapon_type','crystal_type','icon','price','is_magic_weapon','pAtk','mAtk','critRate','pAtkSpd','is_drop','is_sweep','is_craft'];
            $available = $this->getWeaponTableColumns();
            $use = array_values(array_intersect($desired, $available));
            ...
            return $rows;
        }, $this->dbPath, 3600); // cache 1h per page slice
    }
