            ...
            foreach ($errors AS &$err){
                switch ($err['name']){
                    ...
                    case "getCharactersAccount":
                        ...
                        break;
                    case "ErrorConnectLoginDB":
                        $err['err_level'] = 'critical error';
                        $err['message'] = 'Не удалось соединиться с БД логина-сервера.';
                        $err['desc'] = 'Попытка подключиться к БД логина сервера не привела к соединению либо выполнялась слишком долго. Проверьте соединение. Проверьте что разрешен удаленный доступ к Вашей БД от IP SphereAPI. Узнать IP , которому необходимо дать право на соединение можно тут: <a href="/admin/sphereapi">Sphere API</a>';
                        break;
                    case "ErrorConnectGameDB":
                        $err['err_level'] = 'critical error';
                        $err['message'] = 'Отключение получения данных персонажей аккаунта';
                        $err['desc'] = 'Попытка подключиться к БД гейм сервера не привела к соединению либо выполнялась слишком долго. Проверьте соединение. Проверьте что разрешен удаленный доступ к Вашей БД от IP SphereAPI. Узнать IP , которому необходимо дать право на соединение можно тут: <a href="/admin/sphereapi">Sphere API</a>';
                        break;
                    default:
