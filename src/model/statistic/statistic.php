...
class statistic
{
    ...
    public static function get_pvp($server_id = 0)
    {
        ...
        if(isset($statistic['pvp'])){
            return $statistic['pvp'];
        }
        return null;
    }
     ...
     * @throws Exception
     */
    private static function get_data_statistic(
      dir $dir,
    ...
    {
        $statistic = self::getStatistic();
        if(isset($statistic['pk'])){
            return $statistic['pk'];
        }
        return null;

        return \Ofey\Logan22\component\sphere\server::send(type::STATISTIC)['pk'];

        if (isset(self::$pk[$server_id]) && self::$pk[$server_id]) {
            return self::$pk[$server_id];
        }
        try {
            return self::$pk[$server_id] = self::get_data_statistic(
              dir::statistic_pk,
              'statistic_top_pk',
              $server_id,
              second: timeout::statistic_pk->time()
            );
        } catch (Error $e) {
        }
    }
    ...
    {
        $statistic = self::getStatistic();
        if(isset($statistic['online'])){
            return $statistic['online'];
        }
        return null;
        return \Ofey\Logan22\component\sphere\server::send(type::STATISTIC)['online'];

        if (isset(self::$players_online_time[$server_id]) && self::$players_online_time[$server_id]) {
            return self::$players_online_time[$server_id];
        }
        try {
            return self::$players_online_time[$server_id] = self::get_data_statistic(
              dir::statistic_online,
              'statistic_top_onlinetime',
              $server_id,
              second: timeout::statistic_online->time()
            );
        } catch (Error $e) {
        }
    }
    ...
    {
        $statistic = self::getStatistic();
        if(isset($statistic['clan'])){
            return $statistic['clan'];
        }
        return null;
        return \Ofey\Logan22\component\sphere\server::send(type::STATISTIC)['clan'];

        if (isset(self::$clan[$server_id]) && self::$clan[$server_id]) {
            return self::$clan[$server_id];
        }
        try {
            return self::$clan[$server_id] = self::get_data_statistic(
              dir::statistic_clan,
              'statistic_top_clan',
              $server_id,
              second: timeout::statistic_clan->time()
            );
        } catch (Error $e) {
        }
    }
    ...
    {
        $statistic = self::getStatistic();
        if(isset($statistic['castle'])){
            return $statistic['castle'];
        }
        return null;

        return \Ofey\Logan22\component\sphere\server::send(type::STATISTIC)['castle'];

        if (isset(self::$castle[$server_id]) && self::$castle[$server_id]) {
            return self::$castle[$server_id];
        }
        try {
            return self::$castle[$server_id] = self::get_data_statistic(
              dir::statistic_castle,
              'statistic_top_castle',
              $server_id,
              second: timeout::statistic_castle->time()
            );
        } catch (Error $e) {
        }
    }
    ...
    public static function get_players_block($server_id = 0)
    {
        ...
        try {
            return self::$players_block[$server_id] = self::get_data_statistic(
              dir::statistic_block,
              'statistic_top_block',
              $server_id,
              second: timeout::statistic_block->time()
            );
        } catch (Error $e) {
        }
    }
    ...
    public static function get_players_heroes($server_id = 0)
    {
        ...
        try {
            return self::$players_heroes[$server_id] = self::get_data_statistic(
              dir::statistic_heroes,
              'statistic_top_heroes',
              $server_id,
              second: timeout::statistic_heroes->time()
    ...
    {
        return self::get_data_statistic_player(
          dir::statistic_player_info_sub_class,
          'statistic_player_info_sub_class',
          player_name: $player_name,
          server_id: $server_id,
          prepare: [$char_object_id],
          second: timeout::statistic_player_info_sub_class->time()
        );
    }
    ...
    {
        $inventory = self::get_data_statistic_player(
          dir::statistic_player_inventory_info,
          'statistic_player_inventory_info',
          player_name: $player_name,
          server_id: $server_id,
          prepare: [$char_object_id],
          second: timeout::statistic_player_inventory_info->time()
        );
        if ($inventory != null) {
            $lex = [];
    ...
    public static function top_counter($server_id = 0)
    {
        ...
        try {
            return self::$top_counter[$server_id] = self::get_data_statistic(
              dir::statistic_counter,
              'statistic_top_counter',
              $server_id,
              false,
              false,
              second: timeout::statistic_counter->time(),
              playerForbidden: false
            );
        } catch (Error $e) {
        }
    }
    ...
    {
        $clan_info = self::get_data_statistic_clan(
          dir::statistic_clan_data,
          'statistic_clan_data',
          clan_name: $clan_name,
          server_id: $server_id,
          acrossAll: false,
          prepare: [$clan_name],
          second: timeout::statistic_clan_data->time()
        );
        if ( ! $clan_info) {
            redirect::location("/statistic/clan");
        }
        $clan_players = self::get_data_statistic_clan(
          dir::statistic_clan_players,
          'statistic_clan_players',
          clan_name: $clan_name,
          server_id: $server_id,
          acrossAll: true,
          prepare: [$clan_info["clan_id"]],
          second: timeout::statistic_clan_players->time(),
          playerForbidden: true
        );
        $clan_skills  = self::get_data_statistic_clan(
          dir::statistic_clan_skills,
          'statistic_clan_skills',
          clan_name: $clan_name,
          server_id: $server_id,
          crest_convert: false,
          prepare: [$clan_info["clan_id"]],
          second: timeout::statistic_clan_skills->time()
        );
    ...
    private static function get_data_statistic_clan(
      dir $dir,
      string $collection_sql_name,
    ...
    public static function statistic_class($class_id, $prepare = [], $server_id = 0)
    {
        ...
        try {
            return self::$class[$server_id] = self::get_data_statistic_player(
              dir: dir::statistic_class,
              collection_sql_name: 'statistic_top_class',
              player_name: $class_id,
              server_id: $server_id,
              acrossAll: true,
              crest_convert: true,
              prepare: $prepare,
    ...
    private static function getStatistic(): bool|array|null
    {
        ...
        self::$statistic = \Ofey\Logan22\component\sphere\server::send(type::STATISTIC)->getResponse() ?? false;
        return self::$statistic;
    }
