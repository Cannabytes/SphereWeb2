...
class forget
{
    ...
    public static function verification()
    {
        if (config::get_captcha_version("google")) {
            ...
            if ( ! $builder->compare(trim($captcha), $userSessionCaptcha)) {
                board::response("notice", ["message" => lang::get_phrase(295), "ok" => false, "reloadCaptcha" => \Ofey\Logan22\controller\config\config::load()->captcha()->isGoogleCaptcha() == false]);
            }
        }
    ...
    public static function password()
    {
        ...
        if ($data) {
            ...
            if ($second_b < 60 * 10) {
                board::notice(
                  false,
                  "Запрос уже был отправлен. Повторный запрос можно сделать через " . (10 - floor($second_b / 60)) . " минут"
                );
            }
        }
    ...
    public static function generate_string($count = 12): string
    {
