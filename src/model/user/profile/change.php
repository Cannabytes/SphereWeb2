...
class change
{
    ...
    public static function valid_name($name): bool|string
    {
        $ok = true;
        if (2 > mb_strlen($name)) {
            return lang::get_phrase(188);
        }
        if (16 < mb_strlen($name)) {
            return lang::get_phrase(189);
        }
        if ( ! preg_match("/^[a-zA-Z0-9_\-]+$/u", $name)) {
            return lang::get_phrase(190);
        }
     ...
     */
    public static function password_comparison($new_password, $two_password): bool
    {
        $ok = true;
