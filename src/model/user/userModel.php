...
class userModel
{
    ...
    public function __construct(?int $userId)
    {
        ...
        $user = sql::getRow("SELECT `id`, `email`, `password`, `name`, `signature`, `ip`, `date_create`, `date_update`, `access_level`, `donate_point`, `avatar`, `avatar_background`, `timezone`, `country`, `city`, `server_id` FROM `users` WHERE id = ? LIMIT 1", [$userId]);
        if ($user) {
            $this->setIsAuth(true);
            $this->id = $user['id'];
    ...
    public function getPlayers(bool $forcedUpdate = false): ?array
    {
        ...
        if ($this->players === null) {
            $players = sql::getRow("SELECT `id`, `user_id`, `server_id`, `players`, `date` FROM `user_players` WHERE server_id = ? AND user_id = ?;", [
                $this->getServerId(),
                $this->getId(),
            ]);
            if($players===false){
    ...
    {
        $items = sql::getRows("SELECT id, item_id, count, enchant, phrase FROM `warehouse` WHERE server_id = ? AND user_id = ? AND issued = 0", [
            $this->serverId(),
            $this->getId(),
        ]);
        $warehouseArray = [];
        foreach ($items as $item) {
            ...
            $warehouse->setId($item['id'])
                ->setItemId($item['item_id'])
                ->setCount($item['count'])
                ->setEnchant($item['enchant'])
     ...
     */
    function canAffordPurchase(float|int $purchaseAmount): bool
    {
    ...
    public function addLog($type, $phrase, $variables = []): void
    {
        ...
        $file = $trace['file'];
        $line = $trace['line'];
        sql::run("INSERT INTO `logs_all` (`user_id`, `time`, `type`, `phrase`, `variables`, `server_id`, `request`, `method`, `action`, `file`, `line`) VALUES
                        (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    {
        $stmt = sql::run("INSERT INTO `warehouse` (`user_id`, `server_id`, `item_id`, `count`, `enchant`, `phrase`) VALUES (?, ?, ?, ?, ?, ?)", [
            $this->getId(),
            $server_id,
            $item_id,
            $count,
